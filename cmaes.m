...
function [bestmem,bestval,xmean,sigma,FECount,options]=cmaes(fname, func_num, dim, dim_index,bestmem,xmean,sigma,lbound, ubound,oneitermax,options)
...
%     subcmu   = 2 * (submueff-2+1/submueff) / ((subdims+2)^2+submueff);
  ...
  while GenCount < oneitermax
      ...
      for k=1:sublambda
          x(:,k)=bestmem;
          dx=subsigma .* (subB * (subD.* randn(subdims,1)));
          subx(:,k) = subxmean + dx; % m + sig * Normal(0,C) 
          
          indexup=find(subx(:,k)>ubound(dim_index)');
          subx(indexup,k)=ubound(indexup)';
          indexlow=find(subx(:,k)<lbound(dim_index)');
          subx(indexlow,k)=lbound(indexlow)';
          
          x(dim_index,k) = subx(:,k); % this is only for grouping{i}
          val(k) = feval(fname,x(:,k)',func_num);
          FECount = FECount+1;
